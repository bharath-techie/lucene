...
public class TestBKD2 extends LuceneTestCase {
...
//        liveDocs = new Bits.MatchAllBits(reader.maxDoc());
    ...
    public void test2dPoints() throws IOException {
        ...
        search(32, 1000000, 4);
        search(32, 1000000, 8);
        search(32, 1000000, 16);

        index(128, 1000000);
        search(128, 1000000, 1);
        search(128, 1000000, 8);
        search(128, 1000000, 16);
        search(128, 1000000, 32);
        search(128, 1000000, 64);

        index(1024, 1000000);
        search(1024, 1000000, 1);
        search(1024, 1000000, 8);
        search(1024, 1000000, 32);
        search(1024, 1000000, 128);
        search(1024, 1000000, 512);

        index(8192, 1000000);
        search(8192, 1000000, 1);
        search(8192, 1000000, 16);
        search(8192, 1000000, 64);
        search(8192, 1000000, 512);
        search(8192, 1000000, 2048);

        index(1048576, 1000000);
        search(1048576, 1000000, 1);
        search(1048576, 1000000, 16);
        search(1048576, 1000000, 64);
        search(1048576, 1000000, 512);
        search(1048576, 1000000, 2048);
//
//    index(1024 * 1024, 1000000);
//    search(1024 * 1024, 1000000, 1);
//    search(1024 * 1024, 1000000, 16);
//    search(1024 * 1024, 1000000, 64);
//    search(1024 * 1024, 1000000, 512);
//    search(1024 * 1024, 1000000, 2048);
    }

    // Add this method to create range queries
    private static Query getRangeQuery(int cardinality, double selectivity) {
        long min = RANDOM.nextInt(cardinality);
        // Calculate max based on selectivity
    ...
    // Modify the test method to include both point and range queries
    public void test2dPoints1() throws IOException {
        // Test configurations: cardinality, docCount, termCount/selectivity
        int[] cardinalities = {32, 128, 1024, 8192, 1048576};
        int docCount = 100000000;

        // Point queries
        System.out.println("\n=== Point Queries ===");
        for (int cardinality : cardinalities) {
            //index(cardinality, docCount);  // Uncomment if you need to recreate indices
            for (int termCount : new int[]{1, 8, 16, Math.min(cardinality/16, 512)}) {
                searchPoints(cardinality, docCount, termCount);
            }
        }
        ...
        for (int cardinality : cardinalities) {
            //index(cardinality, docCount);  // Uncomment if you need to recreate indices
            for (double selectivity : new double[]{0.01, 0.1, 0.3, 0.5}) {
                searchRanges(cardinality, docCount, selectivity);
            }
        }
    }

    // Add method for point query search
    private static void searchPoints(int cardinality, int docCount, int termCount) throws IOException {
        if (!SEARCH) return;

        String fileName = dictName(cardinality, docCount);
        File tmpDir = new File(System.getProperty("java.io.tmpdir"));
        plaintextDir = new File(tmpDir, "lucene-plaintext-18" + fileName);
        Directory directory = FSDirectory.open(plaintextDir.toPath());
        IndexReader indexReader = DirectoryReader.open(directory);
        IndexSearcher searcher = new IndexSearcher(indexReader);
        setNoCachePolicy(searcher);
        ...
        System.out.println(
                "Point query - " + fileName + ", term count: " + termCount + ", took: " + total / 100 + "ms");
        indexReader.close();
        directory.close();
    }

    // Add method for range query search
    private static void searchRanges(int cardinality, int docCount, double selectivity) throws IOException {
        if (!SEARCH) return;
        ...
        long total = 0;
        for (int i = 0; i < 100; i++) {
            Query query = getRangeQuery(cardinality, selectivity);
            long start = System.currentTimeMillis();
            doSearch(searcher, query);
            long end = System.currentTimeMillis();
            total += end - start;
        ...
        System.out.println(
                "Range query - " + fileName + ", selectivity: " + selectivity + ", took: " + total / 100 + "ms");
        indexReader.close();
        directory.close();
    }

    // Helper method to set no-cache policy
    private static void setNoCachePolicy(IndexSearcher searcher) {
