...
public class OffHeapStarTree implements StarTree {
  ...
  public OffHeapStarTree(IndexInput data) throws IOException {
    ...
    if (MAGIC_MARKER != magicmarker) {
      System.out.println("Magic marker : " + magicmarker);
    ...
    int header = data.readInt(); // header size
    ...
    _dimensionNames.addAll(Arrays.stream(dimensionNames).toList());
    data.readInt(); // num nodes
    // System.out.println("Number of nodes : " + numNodes);
    // System.out.println(data.length());
    RandomAccessInput in =
        data.randomAccessSlice(data.getFilePointer(), data.length() - data.getFilePointer());
  ...
  private void printTreeHelper(
      Map<String, Map<String, String>> dictionaryMap, OffHeapStarTreeNode node, int level)
      throws IOException {
